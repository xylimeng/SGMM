...
for ith_patch = 1:n_patch
    file_mat = sprintf('%s/summary_patch_%d.mat', output_mat_file, ith_patch);
    if exist(file_mat) == 2
        fprintf('%s loaded \n', file_mat)
        load(file_mat)
...
% save('sim2_summary.mat', 'final_two_tail', 'final_left_tail', 'final_right_tail', 'final_count');

% GMM.hard, GMM.soft, SGMM.hard, SGMM.soft, Robust.hard, Robust.soft
addpath('../../SGMM_toolbox')
load('../setting.mat')
% load('sim2_summary.mat')
...
for ith_plot = 1:3
    ...
    saveas(gcf, sprintf('output_png/%s_%s.png', plot_id, plot_names{ith_plot}))
end
