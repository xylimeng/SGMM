...
function [all, two_tail, left_tail, right_tail, count] = unitPatch_mvt(tumor_indicator, convolution_indicator, output_mat, obs_dir, patch_start, patch_end)

addpath(genpath('SGMM_toolbox/'))
load('setting.mat')

% no spatial structure 
B_mat = repmat(par.p, [size(B_mat, 1), 1]);
PI_mat = B_mat; 
...
for ith = patch_start:patch_end
    ...
    ini_mu = par_ini.mu;
    ini_sigma = par_ini.sigma;
    ini_pi = par_ini.p';
    
    save(sprintf('%s/obs%d.mat', obs_dir, ith), 'obs', 'ini_mu', 'ini_sigma', 'ini_pi', 'par_ini');
